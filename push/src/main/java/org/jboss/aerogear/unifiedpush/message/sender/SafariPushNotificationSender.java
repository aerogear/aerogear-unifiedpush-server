...
@SenderType(SafariVariant.class)
public class SafariPushNotificationSender implements PushNotificationSender {

    private final Logger logger = Logger.getLogger(SafariPushNotificationSender.class.getName());

    @Inject
    private ClientInstallationService clientInstallationService;

    /**
     * Sends APNs notifications ({@link org.jboss.aerogear.unifiedpush.message.UnifiedPushMessage}) to all devices, that are represented by
     * the {@link java.util.Collection} of tokens for the given {@link org.jboss.aerogear.unifiedpush.api.iOSVariant}.
     */
    public void sendPushMessage(final Variant variant, final Collection<String> tokens, final UnifiedPushMessage pushMessage, final NotificationSenderCallback callback) {
        ...
        final SafariVariant safariVariant = (SafariVariant) variant;

        final ArrayList<String> urlArgs = (ArrayList<String>)pushMessage.getData().get("url-args");

        PayloadBuilder builder = APNS.newPayload()
                // adding recognized key values
                .alertBody(pushMessage.getAlert()) // alert dialog, in iOS
                .alertTitle(pushMessage.getTitle())
                .alertAction(pushMessage.getAction())
        ...
        ApnsService service = buildApnsService(safariVariant, callback);
        ...
        if (service != null) {
            try {
                ...
                final Set<String> transformedTokens = inactiveTokens;
    ...
    /**
     * Helper method that creates a future {@link java.util.Date}, based on the given ttl/time-to-live value.
     ...
     */
    private ApnsService buildApnsService(SafariVariant safariVariant, final NotificationSenderCallback notificationSenderCallback) {
