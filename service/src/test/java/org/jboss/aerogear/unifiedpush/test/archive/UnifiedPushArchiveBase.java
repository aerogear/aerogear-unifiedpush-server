...
public abstract class UnifiedPushArchiveBase<T extends Archive<T>> extends UnifiedPushArchive<T> {
	...
	public T addAsLibrary(String mavenDependency, String[] findR, String[] replaceR) {
		...
		// Remove old library according to maven dependency name
		// groupId:artifactId
	...
	public T withPushModel() {
		return addMavenDependencies("org.jboss.aerogear.unifiedpush:unifiedpush-push-model");
	}
	...
	public T withModelJPA() {
		return addAsLibrary("org.jboss.aerogear.unifiedpush:unifiedpush-model-jpa",
				...
				new String[] { "META-INF/jpa/test-persistence.xml", "META-INF/jpa/test-data.sql" });
	}

	public T withTestDS() {
		return addAsWebInfResource("META-INF/jpa/test-ds.xml", "test-ds.xml");
	}

	public T withTestResources() {
		return addAsResource("default.properties");
	}
