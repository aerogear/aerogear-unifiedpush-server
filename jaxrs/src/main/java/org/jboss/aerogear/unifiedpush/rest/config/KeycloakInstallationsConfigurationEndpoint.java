...
@Path("/unifiedpush-installations/config")
public class KeycloakInstallationsConfigurationEndpoint implements IKCConfig{

	@GET
	@Produces(MediaType.APPLICATION_JSON)
	public Response configurationFile() {

		String realmName = System.getProperty(KEY_AUTHENTICATION_UPSI_REALM);
		String upsAuthServer = System.getProperty(KEY_AUTHENTICATION_SERVER_URL);

		ObjectMapper mapper = new ObjectMapper(new SystemPropertiesJsonParserFactory());
		mapper.setSerializationInclusion(JsonInclude.Include.NON_DEFAULT);
		AdapterConfig adapterConfig = new AdapterConfig();
		adapterConfig.setRealm(StringUtils.isEmpty(realmName) ? DEFAULT_AUTHENTICATION_UPSI_REALM : realmName);
