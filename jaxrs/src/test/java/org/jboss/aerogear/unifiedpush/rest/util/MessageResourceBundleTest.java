...
public class MessageResourceBundleTest extends RestEndpointTest {
	...
	public void testInternalMessages() {
		...
		assertEquals(messageSource.getMessage(ClickatellSMSSender.MESSAGE_TMPL, new Object[] { "12345" },
				new Locale("es", "ES")), "Su código de verificación para CB4 es: 12345");

		String test = messageSource.getMessage(SendGridEmailSender.MESSAGE_TMPL, new Object[] { "12345" },
				Locale.ENGLISH);
		assertEquals(test,
		...
		String test1 = messageSource.getMessage(SendGridEmailSender.SUBJECT_RESET_KEY, null, Locale.ENGLISH);
		assertEquals(test1, "Email verification code from CB4");
	}
